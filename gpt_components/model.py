...
from utils.common import gather_2d
...
def attn(x, scope, n_state, *, past, hparams):
    ...
    def mask_attn_weights(w):
        # w has shape [batch, heads, dst_sequence, src_sequence], where information flows from gpt_components to dst.
...
class Encoder():
    ...
    def encode_which_outputs_all_layer_h(self, X, h_len, past=None, reuse=tf.AUTO_REUSE):
        with tf.variable_scope(self.scope, reuse=reuse):
            with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
                ...
                h = norm(h, 'ln_f')
                all_h.append(gather_2d(h, tf.expand_dims(final_id, axis=1))[:,0,:])
                target_mask = tf.sequence_mask(h_len, maxlen=tf.shape(h)[1], dtype=tf.float32)
